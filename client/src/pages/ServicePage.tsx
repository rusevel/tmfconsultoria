...
const pages: Record<ServicePageProps["kind"], PageConfig> = {
  tributaria: {
    title: "Consultoria Tributária para Empresas | Cenvara",
    description: "Avalie oportunidades legais, riscos e prioridades tributárias com uma análise sob medida para a sua empresa. Fale com a Cenvara.",
    eyebrow: "consultoria tributária",
    heading: "Decisões tributárias mais claras para uma empresa mais preparada.",
    intro: "A Cenvara analisa o contexto da sua operação para identificar oportunidades previstas na legislação, pontos de atenção e caminhos de planejamento. O diagnóstico considera os dados reais da empresa — sem promessas automáticas.",
  ...
  fiscal: {
    title: "Consultoria Fiscal para Empresas | Cenvara",
    description: "Organize a visão fiscal da sua empresa, identifique riscos e estabeleça prioridades com uma análise técnica e responsável da Cenvara.",
    eyebrow: "consultoria fiscal",
    heading: "Mais controle sobre a operação fiscal. Menos decisões no escuro.",
    intro: "A consultoria fiscal da Cenvara ajuda sua empresa a entender obrigações, processos e pontos de risco para tomar decisões com mais segurança. O escopo é definido conforme a realidade e a documentação disponível.",
...
function DiagnosticArtifact({ page, kind }: { page: PageConfig; kind: ServicePageProps["kind"] }) {
  ...
  return <div className={`diagnostic-artifact artifact-${kind}`} aria-label={page.artifactTitle}>
    <div className="artifact-top"><span className="artifact-live"><span className="pulse-dot" /> leitura ativa</span><span>cenvara / {kind === "tributaria" ? "strategy" : "control"}</span></div>
